...
def main():
    ...
    # ── Special modes (no target IP needed) ───────────────────
    if args.report:
        build_report(root)
        return
    ...
    if args.add_cred:
        if not args.ip:
            err("--add-cred requires a target IP as the first positional argument.")
            sys.exit(1)
    ...
    # ── Normal scan mode ───────────────────────────────────────
    if not args.ip:
        err("Target IP required.")
        parser.print_help()
        sys.exit(1)
 
    ip = args.ip
 
    # Stage 5 — Scaffold first (everything else needs the dirs)
    machine_dir = scaffold(ip, root)
    nmap_dir    = machine_dir / "nmap"
 
    # Stage 4 — AutoRecon background (if requested)
    autorecon_proc = None
    if do_autorecon:
    ...
    # Stage 3 — AD Module (auto-triggered by ports)
